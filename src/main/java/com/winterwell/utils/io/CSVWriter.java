...
public class CSVWriter implements Closeable, Flushable {
	...
	public void write(Object... strings) {
		...
		if (strings.length == 1 && strings[0]!=null && strings[0].getClass().isArray()) {// instanceof String[]) {
		...
		for (int i = 0; i < strings.length; i++) {
			...
			String svi;
			// for double try to avoid E notation
			if (vi instanceof Double) {
				double dvi = (double) vi;
				if (dvi > 0.00000001 && dvi < 10000000L) {
					svi = StrUtils.toNSigFigs((Double)vi, 8);
				} else {
					svi = Double.toString(dvi);
				}
			} else {
				svi = String.valueOf(vi);
			}
			ss[i] = svi;
		}
		write(ss);
