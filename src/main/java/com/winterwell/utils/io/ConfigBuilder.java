...
public class ConfigBuilder {
	...
	public static Object convert(Class<?> type, String string)
		...
		try {
			...
			// string constructor?
			if (ReflectionUtils.isa(type, AString.class)) {
				Object as = type.getConstructor(String.class).newInstance(string);
				return as;
			}
			try {
				Constructor<?> con = type.getConstructor(String.class);
				Object as = con.newInstance(string);
				return as;
			} catch(Exception ex) {
				// oh well
			}
...
//					continue;
	...
	public <S> S get() {
		...
		if (debug) {
			try {
				Log.d(LOGTAG, "Sources: "+getSources());
				for(Field f : source4setFields.keySet()) {
