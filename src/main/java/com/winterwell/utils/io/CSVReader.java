 ...
 * 
 * {@link #asListOfMaps()} provides a handy way to convert csv into key-value maps.
 *
...
public class CSVReader implements Iterable<String[]>, Iterator<String[]>, Closeable, IOneShot {
	...
	*/
	...
	public void reportBadRecord(int lineNumber, String[] record) {
		...
		badRecords++;
		Log.w("csv",
				"(tip:use setNumFields(-1) for flexible records) Bad record at line " + lineNumber + ": "
	 ...
	 * 
	 * Consider wrapping the results in `FlexibleKeyMap` which gives case-insensitive etc key matching.
	 * 
	 * @return
	 */
