...
public class StreamGobbler extends Thread {
	...
	/**
	 * max characters to hold. What is a sensible default??
	 */
	long maxLength = 100*1000000; // 100m chars??
	...
	public void run() {
		try {
			...
			while (!stopFlag) {
				...
				// avoid gobbling too much
				if (maxLength != 0 && stringBuffer.length() > maxLength) {
					Log.w("StreamGobbler", "overflow "+maxLength+" - dropping buffer!");
					clearString();
				}
				if (echo) {
					System.out.print(ch);
	...
	/**
	 * {@link #getString()} + {@link #clearString()} 
	 * (but without the race condition of doing them separately)
	 */
