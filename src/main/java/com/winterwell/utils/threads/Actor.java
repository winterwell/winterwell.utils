...
public class Actor<Msg> {
	...
	private static final Object END = "__END ACTOR__";
	...
	/**
	 * @return true if the queue is empty and this actor is not consuming a message
	 */
	public boolean isIdle() {	
		return q.isEmpty() && ! consuming;
	}
...
//		if (END.equals(msg)) { TODO consistency with consumeBatch()
//			consumeEnd(from);
//			return;
//		}
	...
	/**
	 * pleaseStop() by default
	 * @param from
	 * @throws Exception 
	 */
	protected void consumeEnd(Actor from) throws Exception {	
		Log.d(getName(), "consumeEnd");
		pleaseStop();
	}
	...
	final void loop() {
		...
		while ( ! pleaseStop) {
			...
			try {
				...
				consuming=true; // avoid joinWhenEmpty() returning during processing of msg
				msg = q.poll();
				if (msg == null) {
					consuming=false;
	...
	/**
	 * Send "END" which triggers {@link #consumeEnd(Actor)}
	 * 
	 * @param sender
	 */
	public void sendEnd(Actor sender) {
		send(new Packet(END, sender));
	}
	...
	protected boolean isEnd(Msg msg) {
