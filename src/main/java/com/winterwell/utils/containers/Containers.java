...
public final class Containers  {
	...
	/**
	 * Do records need any special handling?? 
	 * @param record
	 * @return
	 */
	public static Map<String, Object> asMap(Record record) {
		return objectAsMap(record);
	}
	 ...
	 *         accepts. Can contain null if the filter accepts it!
	 */
...
//		return list.stream().filter(a -> a != null).map(fn).filter(b -> Utils.truthy(b)).collect(Collectors.toList());
	...
	/**
	 * @return a new list, filtering out any null or other falsy values (see Utils.truthy() for details)
	 */
	public static <V> List<V> filterFalsy(Collection<V> list) {
		List<V> newMap = filter(list, v -> Utils.truthy(v));
		return newMap;
	}
