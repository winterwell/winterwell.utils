...
public class TimeTest {
	...
	/**
	 * Explore test: Date serialises badly! Time serialises fine
	 */
	@Test
	public void testSerialisation() {
		String xml = XStreamUtils.serialiseToXml(new Time(2023,02,14,9,30,0));
		System.out.println(xml);
		Time t2 = XStreamUtils.serialiseFromXml(xml);
		assert t2.toISOString().equals("2023-02-14T09:30:00Z") : t2.toISOString();
		
		String xml2 = XStreamUtils.serialiseToXml(new Date(2023,02,14,9,30,0));
		System.out.println(xml2);
		Date d2 = XStreamUtils.serialiseFromXml(xml2);
		// This fails!
//		assert new Time(d2).toISOString().equals("2023-02-14T09:30:00Z") : new Time(d2).toISOString();
	}
	...
	public void testFormatMonth() {
		...
		{
			Time t = new Time(2922,9,2);
			String m = t.format("MM");
			assert m.equals("09") : m;
		}
		{
			Time t = new Time(2922,9,2);
			String m = t.format("MMM");
			assert m.equals("Sept") : m;
		}
		{
			Time t = new Time(2922,9,2);
			String m = t.format("MMMMM");
			assert m.equals("September") : m;
		}
	}
	...
	public void testISO8601Format() {		
		Time tJan = new Time(2023,02,01).minus(TUnit.SECOND);
		System.out.println(tJan.toISOString());
		System.out.println(tJan.getTime());
