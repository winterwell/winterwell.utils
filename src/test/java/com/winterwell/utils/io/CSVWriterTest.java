...
public class CSVWriterTest extends TestCase {
	...
	public void testNumericalWrite() {
		...
		String txt = FileUtils.read(f);
		assert Pattern.compile("1,2,3(.0)?,4(.0)?\n").matcher(txt).matches();

		FileUtils.delete(f);
	}

	@Ignore // fails currently
	public void testNumericalWriteSmall() {
		File f = new File("test/csvnumwriter.small.txt");

		CSVWriter w = new CSVWriter(f, ',');
		w.write(new Object[] {3, (double) 0.0000012, 0.00000000567 });
		w.close();

		String txt = FileUtils.read(f);
//		assertEquals("3,0.0000012,0.00000000567\n", txt);
		assertEquals("3,0.0000012,5.67E-9\n", txt);
